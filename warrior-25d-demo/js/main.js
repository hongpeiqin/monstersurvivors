...
import { Hero, Enemy, SpatialHash, DIRECTIONS, clamp, normalize, quantizeDirection } from './entities.js';
...
class FrameScheduler {
  constructor() {
    ...
    this.frame = 0; this.ema = 16.7; this.stable = 0; this.lastSwitch = 0;
  }
  sample(ms) {
    this.frame++;
    ...
    else if (this.ema > 20.5) next = this.mode === 'low' && this.ema < 22 ? 'balanced' : 'balanced';
    else if (this.ema < 17.4) next = 'high';
    if (next !== this.mode) {
      this.mode = next; this.profile = PROFILES[next]; this.lastSwitch = this.frame; this.stable = 0; return true;
    }
    return false;
  }
}
...
function startAction(kind) {
  ...
  let combo = 1;
  if (kind === 'attack') {
    combo = hero.comboWindow > 0 ? hero.combo % 3 + 1 : 1;
    hero.combo = combo; hero.comboWindow = .58;
  } else {
    hero.combo = 0; hero.comboWindow = 0; cooldowns[kind] = cooldownMax[kind];
  }
...
function emitAttackVisual(kind, combo) {
  ...
  if (kind === 'attack') {
    ...
    visuals.ring({ x: hero.x, y: hero.y, radius: 68, width: 14, life: .35, color: '#79e7ff' });
  }
}
...
function resolveAttack(attack) {
  ...
  for (const enemy of candidates) {
    ...
    if (attack.kind === 'spin') hit = dist <= config.radius + enemy.radius;
      ...
      const dot = clamp(n.x * hero.dir.x + n.y * hero.dir.y, -1, 1);
      hit = Math.acos(dot) <= config.cone * .5;
    }
    if (!hit) continue;
    const force = config.heavy ? 260 : 145;
    const n = dist ? { x: dx / dist, y: dy / dist } : hero.dir;
    ...
    particles.burst(config.heavy ? 'shard' : 'spark', enemy.x, enemy.y, {
      count: config.heavy ? 24 : 13, color: config.heavy ? ['#fff3a8', '#7ff7e1', '#ffffff'] : ['#d9ffff', '#76efd8'],
      angle: Math.atan2(n.y, n.x), spread: 1.25, speedMin: 70, speedMax: config.heavy ? 300 : 210,
      lifeMin: .22, lifeMax: .58, important: true, z: 24
    });
    if (died) {
      kills++; audio.death();
...
for (const button of dom.skills) {
  ...
  button.addEventListener('pointerdown', event => {
    ...
    if (kind === 'attack') { input.attackHeld = true; startAction('attack'); } else startAction(kind);
  });
...
addEventListener('keydown', event => {
  ...
  if (!event.repeat) {
    ...
    if (event.code === 'KeyQ') startAction('slash');
    if (event.code === 'KeyE') startAction('spin');
    if (event.code === 'KeyR') startAction('dash');
...
function bindSettings() {
  ...
  const updateLabels = () => { dom.bgmValue.value = `${Math.round(settings.bgm * 100)}%`; dom.sfxValue.value = `${Math.round(settings.sfx * 100)}%`; };
  updateLabels();
  dom.bgm.addEventListener('input', () => { settings.bgm = Number(dom.bgm.value); updateLabels(); saveSettings(); audio.applySettings(); });
  dom.sfx.addEventListener('input', () => { settings.sfx = Number(dom.sfx.value); updateLabels(); saveSettings(); audio.applySettings(); audio.ensure(); });
  ...
  dom.particles.addEventListener('change', () => {
    settings.particles = dom.particles.checked; saveSettings(); particles.setEnabled(settings.particles);
    showToast(settings.particles ? '粒子特效已开启' : '粒子特效已关闭，存量已清空');
  });
}
bindSettings();
...
function updateEnemyAiShard(now) {
  ...
  for (const enemy of enemies) {
    ...
    for (const other of grid.nearby(enemy.x, enemy.y, 74)) {
      ...
      const ox = enemy.x - other.x, oy = enemy.y - other.y, od = Math.hypot(ox, oy) || 1;
      const target = enemy.radius + other.radius + 12;
...
function updateEnemies(dt) {
  ...
  if (scheduler.frame % 45 === 0) {
    for (let i = enemies.length - 1; i >= 0; i--) {
      ...
      const far = Math.hypot(enemy.x - hero.x, enemy.y - hero.y) > 1900;
...
function update(dt) {
  ...
  hero.update(dt, movementVector());
  updateAttack(dt);
  updateEnemies(dt);
  visuals.update(dt);

  if (!hero.dead && input.attackHeld && !hero.attack) startAction('attack');
  if (hero.dead && hero.respawn <= 0) {
    const oldKills = kills; hero.reset(); hero.x = camera.x; hero.y = camera.y; particles.clear(); showToast(`重新投入战场 · 已击破 ${oldKills}`);
  }
  ...
  camera.x += (hero.x - camera.x) * follow; camera.y += (hero.y - camera.y) * follow;
  if (visuals.shake > 0) { shakeX = (Math.random() * 2 - 1) * visuals.shake; shakeY = (Math.random() * 2 - 1) * visuals.shake; }
  else { shakeX = shakeY = 0; }

  hudTimer -= dt; toastTimer -= dt;
  if (toastTimer <= 0) dom.toast.classList.remove('show');
  if (hudTimer <= 0) { updateHud(); hudTimer = .12; }
}
...
function updateHud() {
  ...
  dom.skills.forEach(button => {
    const kind = button.dataset.action; const max = cooldownMax[kind] || 0; const remain = cooldowns[kind] || 0;
    button.classList.toggle('cooling', remain > 0); button.querySelector('.cooldown').style.height = max ? `${remain / max * 100}%` : '0%';
  });
}

function drawDebug() {
  if (!debugVisible) return;
  ctx.save(); ctx.fillStyle = 'rgba(2,8,12,.72)'; ctx.fillRect(12, height - 110, 252, 94);
  ctx.strokeStyle = 'rgba(132,238,221,.3)'; ctx.strokeRect(12.5, height - 109.5, 251, 93);
  ctx.fillStyle = '#c8fff2'; ctx.font = '11px ui-monospace,monospace';
  const lines = [
    `build ${BUILD}  frame ${scheduler.frame}`, `EMA ${scheduler.ema.toFixed(2)}ms  ${Math.round(1000 / scheduler.ema)}fps`,
    `mode ${scheduler.mode}  AI/${scheduler.profile.aiSlices}  FX/${scheduler.profile.particleSlices}`,
    `enemy ${enemies.length}  particle ${particles.items.length}/${scheduler.profile.maxParticles}`,
    `particle switch ${settings.particles ? 'ON' : 'OFF'}  drawn ${particles.stats.drawn}`
  ];
...
function render() {
  ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
  drawGround();
  visuals.drawWorld(ctx, project, settings.particles);

  const drawables = [];
  for (const enemy of enemies) {
    const p = project(enemy.x, enemy.y, 0);
    if (p.x < -90 || p.x > width + 90 || p.y < -100 || p.y > height + 90) continue;
    drawables.push({ y: enemy.y, type: 0, enemy });
  }
  drawables.push({ y: hero.y, type: 1 });
  ...
  particles.draw(ctx, project, performance.now(), { width, height });
  visuals.drawText(ctx, project);
  drawDebug();
}
...
window.__WARRIOR_DEMO__ = {
  build: BUILD,
  getState: () => ({
    particlesEnabled: particles.enabled, particleCount: particles.items.length, mode: scheduler.mode,
    aiSlices: scheduler.profile.aiSlices, particleSlices: scheduler.profile.particleSlices,
    enemies: enemies.filter(enemy => !enemy.dead).length, frameMs: Number(scheduler.ema.toFixed(2)), kills
  }),
  ...
  stress(count = 90) {
    ...
    while (enemies.filter(enemy => !enemy.dead).length < target) {
      const angle = Math.random() * TAU, radius = 180 + Math.random() * 900;
      const enemy = new Enemy(nextEnemyId++, hero.x + Math.cos(angle) * radius, hero.y + Math.sin(angle) * radius, Math.random() < .12 ? 1 : 0);
      enemies.push(enemy);
    }
